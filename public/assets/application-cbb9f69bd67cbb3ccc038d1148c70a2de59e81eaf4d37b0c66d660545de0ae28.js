...
document.addEventListener("turbo:load", function () {
  ...
  // Add necessary attributes to the swiper container
  swiperContainer.setAttribute('direction', 'horizontal');
  swiperContainer.setAttribute('loop', 'false');
  swiperContainer.setAttribute('slides-per-view', '1');
  swiperContainer.setAttribute('navigation', 'false');
  swiperContainer.setAttribute('pagination', 'false');
  swiperContainer.setAttribute('keyboard', 'true');
  swiperContainer.setAttribute('effect', 'cards');
  
  // Optional: You can set inline styles if needed
  swiperContainer.style.setProperty('--swiper-navigation-color', '#333');
  swiperContainer.style.setProperty('--swiper-pagination-color', '#333');
  swiperContainer.style.width = '100%';
  swiperContainer.style.maxWidth = '600px';
  swiperContainer.style.margin = '0 auto';

  // Insert the swiper container into the DOM (e.g., inside a specific element)
  document.body.appendChild(swiperContainer); // Or append to a specific element like document.querySelector('#someContainer')
  ...
  const swiper = new Swiper(swiperContainer, {
    ...
    if (!blankSlideAdded && swiper) {
      try {
        const blankSlide = `
          <swiper-slide style="padding: 60px; border: 3px solid #ccc; border-radius: 16px; background-color: #f9f9f9;">
          </swiper-slide>`;
        swiper.prependSlide(blankSlide); // Add the blank slide
        blankSlideAdded = true;
        console.log("Blank slide added successfully.");
      } catch (error) {
        console.error("Error adding blank slide:", error);
      }
    } else if (!swiper) {
      console.error("Swiper instance is undefined. Cannot add slides.");
    }
