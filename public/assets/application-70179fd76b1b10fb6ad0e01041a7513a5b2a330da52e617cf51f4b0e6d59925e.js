...
document.addEventListener("turbo:load", function () {
  ...
  if (swiperContainer) {
    ...
    // If swiper is not already initialized, create a new instance
    if (!swiper) {
      ...
      // Ensure Swiper is initialized before adding slides
    if (swiper) {
      const blankSlide = `
        <swiper-slide style="padding: 60px; border: 3px solid #ccc; border-radius: 16px; background-color: #f9f9f9;">
        </swiper-slide>
      `;
       // Safely add the slide
      try {
        swiper.prependSlide(blankSlide);
        console.log("Blank slide added successfully.");
      } catch (error) {
        console.error("Failed to add blank slide:", error);
      }
    } else {
      console.error("Swiper instance is not defined. Cannot add slide.");
    }
