    ...
    swiperContainer.addEventListener('swiperslidechange', (event) => {
      ...
      if (activeIndex !== lastSeenIndex) {
        ...
        if (direction === 'right') {
          ...
          // Manually slide to the next slide after a short delay
          setTimeout(() => {
            swiper.slideNext(); // Move to the next slide after removing the current one
          }, 500); // Delay to match the overlay display time
        }
      }
    });
  } else {
